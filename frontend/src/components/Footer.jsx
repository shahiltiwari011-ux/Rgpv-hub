...
export default function Footer() {
  ...
  return (
    <footer className="projectx-footer">
      ...
      <style jsx>{`
        .projectx-footer { background: #03040a; border-top: 1px solid rgba(255,255,255,0.05); padding: 5rem 1.5rem 2rem; position: relative; overflow: hidden; }
        ...
        .footer-logo { font-family: 'Syne', sans-serif; font-size: 2rem; font-weight: 800; letter-spacing: -2px; margin: 0 0 1.5rem; }
        .footer-logo span { color: #3b82f6; }
        .footer-tagline { color: #64748b; line-height: 1.6; font-size: 0.95rem; max-width: 350px; }
        
        .footer-badges { display: flex; gap: 0.75rem; margin-top: 2rem; }
        .badge { font-size: 0.6rem; font-weight: 900; color: #3b82f6; background: rgba(59, 130, 246, 0.1); border: 1px solid rgba(59, 130, 246, 0.2); padding: 0.3rem 0.8rem; border-radius: 2rem; letter-spacing: 1px; }

        .links-zone { display: grid; grid-template-columns: 1fr 1fr; gap: 2rem; }
        .link-group h4 { font-size: 0.75rem; font-weight: 900; color: #475569; margin-bottom: 1.5rem; letter-spacing: 2px; }
        .link-group ul { list-style: none; padding: 0; margin: 0; }
        .link-group li { margin-bottom: 0.75rem; }
        .link-group a { text-decoration: none; color: #94a3b8; font-size: 0.9rem; font-weight: 600; transition: 0.3s; }
        .link-group a:hover { color: #3b82f6; padding-left: 5px; }

        .humour-zone h4 { font-size: 0.75rem; font-weight: 900; color: #475569; margin-bottom: 1.5rem; letter-spacing: 2px; }
        .joke-card { background: rgba(255,255,255,0.02); border: 1px solid rgba(255,255,255,0.05); border-radius: 1.5rem; padding: 2rem; position: relative; overflow: hidden; min-height: 120px; display: flex; align-items: center; justify-content: center; text-align: center; }
        .joke-card p { margin: 0; font-size: 0.9rem; color: #cbd5e1; font-weight: 500; font-style: italic; line-height: 1.5; }
        .joke-progress { position: absolute; bottom: 0; left: 0; width: 100%; height: 2px; background: rgba(255,255,255,0.05); }
        .progress-bar { height: 100%; background: #3b82f6; animation: progress 8s linear infinite; }
        @keyframes progress { from { width: 0%; } to { width: 100%; } }

        .footer-bottom { border-top: 1px solid rgba(255,255,255,0.05); padding-top: 2rem; display: flex; justify-content: space-between; align-items: center; color: #475569; font-size: 0.75rem; font-weight: 700; }
        @media (max-width: 600px) { .footer-bottom { flex-direction: column; gap: 1rem; text-align: center; } }
        .dot { margin: 0 0.5rem; color: #1e293b; }
        .made-by span { color: #3b82f6; }
      `}</style>
    </footer>
  );
}
